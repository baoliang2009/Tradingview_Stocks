...
3. ✅ **自动止损功能**：支持自定义止损比例，有效控制风险（v2.3新增）
4. ✅ **详细性能指标**：胜率、盈亏比、夏普比率、最大回撤、止损率等
5. ✅ **结果导出**：自动保存CSV格式的汇总和详细交易记录
6. ✅ **可视化对比**：不同质量阈值的性能对比表格
...
# 使用5%更严格的止损
python3 backtest.py --stop-loss 0.05
```
...
| `--stop-loss` | float | 0.10 | 止损比例（如0.10表示-10%）|
...
2. **卖出信号**：使用开盘价卖出
3. **止损机制**：使用当天最低价检查止损，触发后立即平仓（v2.3新增）
4. **持仓管理**：满仓操作（可调整 position_size）
5. **成本计算**：
   - 买入成本 = 买入价 × (1 + 滑点 + 手续费)
   - 卖出所得 = 卖出价 × (1 - 滑点 - 手续费)
...
------------------------------------------------------------------------------------------------------------
0         120        58.33      1.85        98.50       -22.30      45.0     1.25       0.85      
50        95         62.11      2.45        135.20      -18.50      38.0     1.42       1.05      
60        68         65.22      2.95        148.60      -16.20      35.0     1.58       1.22      
70        45         68.89      3.25        156.30      -15.60      33.0     1.71       1.35      
80        25         72.00      3.85        168.50      -12.80      28.0     1.92       1.58      
```
...
- 收益率、持有天数
- 信号质量、交易状态
- **退出原因**（止损/信号/持有中）← v2.3新增

> 📘 **止损功能详细说明**: 查看 [止损功能指南](STOP_LOSS_GUIDE.md)
